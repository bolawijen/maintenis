...
import {ScrollTopButton} from '@maintenis/tilik-sdk/Component/ScrollTop';
import {YiiIcon} from '@maintenis/tilik-sdk/Component/SvgIcon/YiiIcon';
...
const BreadcrumbsWrapper = ({toggleDrawer}: MyBreadcrumbsType) => {
    ...
    return (
        <Breadcrumbs sx={{my: 2}}>
            ...
            {cumulativeBreadcrumbs.map((item, i) =>
                typeof item == 'string' ? (
                    <Link underline="hover" color="inherit" href={item} key={i}>
                        {item}
                    </Link>
                ) : (
                    <Link underline="hover" color="inherit" href={item.href} key={i}>
                        {item.title}
                    </Link>
                ),
            )}
